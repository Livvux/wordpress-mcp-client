...
function PureMultimodalInput({
  ...
  const handleWordPressConnectionChange = useCallback(
    (connected: boolean, siteUrl?: string) => {
      setIsWordPressConnected(connected);
      if (siteUrl) {
        setWordPressSiteUrl(siteUrl);
      } else if (!connected) {
        setWordPressSiteUrl('');
      }
    },
  ...
  // Check WordPress connection status on mount
  useEffect(() => {
    const checkConnection = async () => {
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          setIsWordPressConnected(data.connected);
          setWordPressSiteUrl(data.siteUrl || '');
        }
      } catch (error) {
  ...
  return (
    <div className="relative w-full flex flex-col gap-4">
      ...
      <div className="absolute bottom-0 left-0 p-2 w-fit flex flex-row justify-start gap-1">
        <AttachmentsButton fileInputRef={fileInputRef} status={status} />
        <WordPressSettingsButton
          status={status}
          onConnectionChange={handleWordPressConnectionChange}
          isConnected={isWordPressConnected}
          siteUrl={wordPressSiteUrl}
        />
        <ToolsModal status={status} isConnected={isWordPressConnected} />
      </div>
...
function extractSiteName(url: string): string {
  ...
  try {
    ...
    // Remove 'www.' if present and return the domain name
...
function PureWordPressSettingsButton({
  status,
  onConnectionChange,
  isConnected,
  siteUrl,
    ...
    <SettingsModal
      ...
      trigger={
        <div className="relative">
          <Button
            data-testid="wordpress-settings-button"
          ...
          {isConnected && siteName && (
            <div className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 translate-y-full z-10">
              <div className="px-2 py-1 text-xs bg-blue-600 text-white rounded-md whitespace-nowrap shadow-sm">
                {siteName}
              </div>
            </div>
          )}
...
const WordPressSettingsButton = memo(PureWordPressSettingsButton);
