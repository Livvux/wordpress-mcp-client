...
async function ConnectionStatus() {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_APP_URL || ''}/api/mcp/connection/status`,
    {
      cache: 'no-store',
    },
  );
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-1 text-sm">
        ...
        <div>
          <span className="text-zinc-500 dark:text-zinc-400">Write mode: </span>
...
export default async function WPPluginOnboardingPage() {
  ...
  const originRuntime =
    originEnv ||
    getOriginFromHeaders((await headers()) as unknown as Headers) ||
    '';
    ...
    <div className="space-y-8">
      <div className="space-y-1">
        ...
        <p className="text-sm text-zinc-600 dark:text-zinc-400">
          Download the plugin and connect your WordPress site without manually
          entering tokens.
        </p>
      </div>
        ...
        <Card className="md:col-span-2">
          ...
          <CardContent className="space-y-3">
            <div className="text-sm">
              Get the latest WP Cursor plugin ZIP and install it in your
              WordPress admin.
            </div>
            ...
            <div className="text-xs text-zinc-500 dark:text-zinc-400">
              WordPress → Plugins → Add New → Upload Plugin → Choose File →
      ...
      <Card>
        ...
        <CardContent className="space-y-4 text-sm">
          <div>
            In your WordPress admin, go to Tools → WP Cursor → Connect to App.
            Use this App URL:
          </div>
          <div className="flex items-center gap-2">
            <code className="px-2 py-1 rounded bg-zinc-100 dark:bg-zinc-800 text-xs break-all">
              {originRuntime || 'http://localhost:3000'}
            </code>
            <CopyButton
              variant="outline"
              text={originRuntime || 'http://localhost:3000'}
            >
              Copy
            </CopyButton>
          </div>
          <div className="text-xs text-zinc-500 dark:text-zinc-400">
            The plugin will redirect back here and your connection will be saved
            to your account.
          </div>
        </CardContent>
      </Card>
      ...
      <Alert>
        <AlertDescription>
          Note: Linking only works when you are signed in. Connections are saved
          to your account and persist across sessions.
        </AlertDescription>
      </Alert>
    </div>
  );
