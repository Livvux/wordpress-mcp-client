...
export async function loadWordPressTools(config: WordPressToolConfig) {
  ...
  try {
    ...
    for (const mcpTool of mcpTools) {
      ...
      const isWriteOperation = mcpTool.kind === 'action' || 
        ['create', 'update', 'delete', 'edit', 'publish', 'trash'].some(
          op => mcpTool.name.toLowerCase().includes(op)
        );
      ...
      aiTools[mcpTool.name] = tool({
        ...
        execute: async (args: any) => {
          try {
            ...
            console.error(`Error executing WordPress tool ${mcpTool.name}:`, error);
            throw error;
          }
        },
      });
...
export function categorizeTools(tools: MCPTool[]): Record<string, MCPTool[]> {
  ...
  for (const tool of tools) {
    ...
    } else if (name.includes('media') || name.includes('image') || name.includes('attachment')) {
    ...
    } else if (name.includes('woo') || name.includes('product') || name.includes('order')) {
